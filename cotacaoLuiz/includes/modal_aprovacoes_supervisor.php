<?php
// filepath: c:\Xampp\htdocs\cotacao\includes\modal_aprovacoes_supervisor.php
?>
<link rel="stylesheet" href="assets/css/modal_aprovacoes_supervisor.css">
...
<div id="visualizacao-melhor-pagamento" class="visualizacao-container" style="display: none;">
    <div class="detalhes-cotacao">
        ...
        <div id="analise-acoes" class="analise-acoes-footer">
            <button class="btn-grande btn-aprovar-grande" onclick="liberarParaGerencia(${data.id})">
                <i class="fas fa-check"></i> Liberar para Gerência
            </button>
...
function analisarCotacao(id) {
    ...
    console.log('Número final de rodadas:', numeroRodadas);
        ...
        if (data.status === 'aguardando_aprovacao_supervisor' || data.status === 'renegociacao') {
            acoesDiv.innerHTML = `
                <button class="btn-grande btn-aprovar-grande" onclick="liberarParaGerencia(${data.id})">
                    <i class="fas fa-check"></i> Liberar para Gerência
                </button>
...
function renderizarMelhorPreco() {
    ...
    Object.entries(itensPorProduto).forEach(([produtoNome, dados]) => {
        ...
        itensMelhorPreco.forEach(item => {
            ...
            // Calcular saving para todos os itens
...
function renderizarMelhorEntrega() {
    ...
    cotacaoData.itens.forEach(item => {
        ...
        if (!itensPorFornecedor[fornecedorNome]) {
            itensPorFornecedor[fornecedorNome] = {
                melhorDataEntrega: item.data_entrega_fn || '9999-12-31',
                itens: []
            };
        }
        const dataAtual = item.data_entrega_fn || '9999-12-31';
        if (dataAtual < itensPorFornecedor[fornecedorNome].melhorDataEntrega) {
            itensPorFornecedor[fornecedorNome].melhorDataEntrega = dataAtual;
        }
    ...
    // Ordenar fornecedores pela melhor data de entrega
    const fornecedoresOrdenados = Object.entries(itensPorFornecedor)
        .sort(([, a], [, b]) => a.melhorDataEntrega.localeCompare(b.melhorDataEntrega));
    
    // Renderizar todos os produtos de cada fornecedor
    fornecedoresOrdenados.forEach(([fornecedorNome, dados]) => {
        // Renderizar cada item do fornecedor
...
function renderizarMelhorPagamento() {
    ...
    cotacaoData.itens.forEach(item => {
        ...
        // Comparar prazos de pagamento
        const prazoAtual = item.prazo_pagamento || '';
        const prazoAtualNumeros = prazoAtual.split('/').map(n => parseInt(n) || 0);
        const melhorPrazoNumeros = itensPorFornecedor[fornecedorNome].melhorPrazoPagamento.split('/').map(n => parseInt(n) || 0);
        
        // Se o prazo atual tiver mais dias ou mais opções de prazo, atualiza
        if (prazoAtualNumeros.length > melhorPrazoNumeros.length || 
            Math.max(...prazoAtualNumeros) > Math.max(...melhorPrazoNumeros)) {
            itensPorFornecedor[fornecedorNome].melhorPrazoPagamento = prazoAtual;
        }
    ...
    // Ordenar fornecedores pelo melhor prazo de pagamento
    const fornecedoresOrdenados = Object.entries(itensPorFornecedor)
        .sort(([, a], [, b]) => {
            const prazoA = a.melhorPrazoPagamento.split('/').map(n => parseInt(n) || 0);
            const prazoB = b.melhorPrazoPagamento.split('/').map(n => parseInt(n) || 0);
            return Math.max(...prazoB) - Math.max(...prazoA); // Ordena do maior para o menor prazo
        });
    ...
    html += `
        <tr class="total-row">
            ...
            <td colspan="2"><strong>R$ ${valorTotal.toFixed(4).replace('.', ',')}</strong></td>
            <td></td>
        </tr>
    `;
    ...
    Object.entries(produtosAgrupados).forEach(([nomeProduto, itens]) => {
        ...
        try {
            ...
            itens.forEach((item, index) => {
                const ehMelhorPreco = item.valor_unitario === melhorPrecoItem.valor_unitario;
...
// Adicione esta função no bloco <script> do arquivo modal_aprovacoes_supervisor.php
...
// Adicionar esta função no arquivo modal_aprovacoes_supervisor.php
...
function renderizarResumoComparativo() {
    ...
    Object.values(itensPorProduto).forEach(grupo => {
        ...
        const itensPorPrazoPagamento = itensComPrazoPagamento.length > 0 ?
            [...itensComPrazoPagamento].sort((a, b) => {
                const prazosA = a.prazo_pagamento.split('/').length;
                const prazosB = b.prazo_pagamento.split('/').length;
                return prazosB - prazosA; // Ordem decrescente (mais opções é melhor)
            }) : [];
        const melhorPrazoPagamento = itensComPrazoPagamento.length > 0 ? itensPorPrazoPagamento[0] : null;
        
        // Renderizar cada item com o melhor preço
        itensComMelhorPreco.forEach(item => {
            html += `
                <tr class="indicador-preco">
                    ...
                    <td>${grupo.unidade}</td>
                    <td class="melhor-preco">R$ ${formatarNumero(item.valor_unitario)}</td>
                    <td>${item.fornecedor_nome}</td>
                    <td class="melhor-entrega">${item.prazo_entrega || '-'}</td>
                    <td>${item.fornecedor_nome}</td>
                    <td class="melhor-pagamento">${item.prazo_pagamento || '-'}</td>
                    <td>${item.fornecedor_nome}</td>
                </tr>
            `;
        });
        
        // Se houver melhor prazo de entrega diferente do melhor preço
        if (melhorPrazoEntrega) {
            const itensComMelhorPrazoEntrega = itensComPrazoEntrega.filter(item => {
                const diasItem = parseInt(item.prazo_entrega.match(/\d+/)?.[0] || 999);
                const diasMelhor = parseInt(melhorPrazoEntrega.prazo_entrega.match(/\d+/)?.[0] || 999);
        ...
        // Se houver melhor prazo de pagamento diferente dos anteriores
        if (melhorPrazoPagamento) {
            const itensComMelhorPrazoPagamento = itensComPrazoPagamento.filter(item => {
                const prazosItem = item.prazo_pagamento.split('/').length;
                const prazosMelhor = melhorPrazoPagamento.prazo_pagamento.split('/').length;
                return prazosItem === prazosMelhor;
            });
            
            itensComMelhorPrazoPagamento.forEach(item => {
                if (!itensComMelhorPreco.some(i => i.fornecedor_nome === item.fornecedor_nome) &&
                    (!melhorPrazoEntrega || !itensComPrazoEntrega.some(i => i.fornecedor_nome === item.fornecedor_nome))) {
                    html += `
                        <tr class="indicador-pagamento">
                            <td>${grupo.nome}</td>
                            <td>${parseFloat(item.quantidade).toFixed(2)}</td>
...
function liberarParaGerencia(id) {
    if (confirm('Tem certeza que deseja liberar esta cotação para análise da gerência?')) {
        // Coletar estado atual dos checkboxes de renegociação
        const produtosRenegociar = Array.from(document.querySelectorAll('.checkbox-renegociar:checked'))
            .map(cb => ({
        ...
        // Primeiro, atualizar os produtos para renegociação
        fetch('api/cotacoes.php', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                id: id,
                status: 'renegociacao',
                produtos_renegociar: produtosRenegociar,
                motivo_renegociacao: 'Atualização automática ao liberar para gerência'
            })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Se a atualização dos produtos foi bem sucedida, liberar para gerência
                return fetch('api/cotacoes.php', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        id: id,
                ...
                throw new Error(data.message || 'Erro ao atualizar produtos para renegociação');
            }
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                alert('Cotação liberada para gerência com sucesso!');
                window.location.reload();
            } else {
                alert(data.message || 'Erro ao liberar para gerência');
            }
        })
        .catch(error => {
            console.error('Erro:', error);
