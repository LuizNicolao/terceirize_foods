...
export const useProdutosPerCapitaFilters = () => {
  const [filtros, setFiltros] = useState({
    search: '',
    ativo: 'true', // Default para produtos ativos
    per_capita: '', // Filtro de tipo de per capita
    tipo_produto: '', // Filtro por tipo de produto
    page: 1,
    limit: 10
  });
  ...
  const clearFiltros = useCallback(() => {
    setFiltros({
      search: '',
      ativo: 'true', // Default para produtos ativos
      per_capita: '', // Reset per capita filter
      tipo_produto: '', // Reset tipo produto filter
      page: 1,
      limit: 10
    });
  }, []);
