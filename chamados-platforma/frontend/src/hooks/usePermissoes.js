...
export const usePermissoes = () => {
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      const permissoesArray = Object.entries(editingPermissions).map(([tela, perms]) => {
        ...
        if (tela === 'chamados') {
          basePerms.pode_assumir = perms.pode_assumir ? 1 : 0;
          basePerms.pode_concluir = perms.pode_concluir ? 1 : 0;
          basePerms.pode_reabrir = perms.pode_reabrir ? 1 : 0;
          basePerms.pode_atribuir = perms.pode_atribuir ? 1 : 0;
        } else {
          // Para outras telas, definir como 0
          basePerms.pode_assumir = 0;
          basePerms.pode_concluir = 0;
          basePerms.pode_reabrir = 0;
          basePerms.pode_atribuir = 0;
        }
  ...
  // Marcar/desmarcar todas as permissões de uma tela específica
  const handleToggleAllPermissionsForScreen = (tela, value) => {
    setEditingPermissions(prev => {
      const currentPerms = prev[tela] || {};
  ...
  // Marcar/desmarcar uma permissão específica em todas as telas
  const handleTogglePermissionForAllScreens = (action, value) => {
    setEditingPermissions(prev => {
      const newPerms = { ...prev };
      const allScreens = Object.keys(newPerms);
      
      allScreens.forEach(tela => {
        newPerms[tela] = {
          ...newPerms[tela],
          [action]: value
        };
  ...
  return {
    ...
    handleToggleAllPermissionsForScreen,
    handleTogglePermissionForAllScreens,
    handleExpandGroup,
