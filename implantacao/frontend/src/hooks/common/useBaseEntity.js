...
export const useBaseEntity = (entityName, service, options = {}) => {
  ...
  }, [entityName, service, pagination.currentPage, pagination.itemsPerPage, filters.searchTerm, filters.statusFilter, enableStats]);
  ...
  return {
    ...
    handleOpenModal: (item, isView = false) => {
      if (isView) {
        modal.handleView(item);
      } else {
        modal.handleEdit(item);
      }
    },
