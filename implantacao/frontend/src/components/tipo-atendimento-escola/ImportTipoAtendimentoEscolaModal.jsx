...
const ImportTipoAtendimentoEscolaModal = ({ isOpen, onClose, onImportSuccess }) => {
  ...
  const handleDownloadModelo = async () => {
    try {
      setLoading(true);
      const response = await TipoAtendimentoEscolaService.baixarModelo();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await TipoAtendimentoEscolaService.importarExcel(formData);

      if (response.success) {
        setResultado(response.data);
        toast.success(response.message || 'Importação realizada com sucesso!');
        
        // Se teve sucesso parcial ou total, chamar callback
        if (response.data && response.data.sucesso && response.data.sucesso.length > 0) {
          setTimeout(() => {
            onImportSuccess();
          }, 2000);
        }
      } else {
        setErro(response.error || 'Erro ao importar planilha');
      ...
      toast.error(error.response?.data?.message || 'Erro ao processar importação');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Importar Tipo de Atendimento por Escola"
      size="3xl"
    >
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
            ...
            <li>Os campos <strong>escola_id</strong> e <strong>tipos_atendimento</strong> são obrigatórios</li>
            <li>Os tipos de atendimento válidos são: <strong>lanche_manha</strong>, <strong>almoco</strong>, <strong>lanche_tarde</strong>, <strong>parcial_manha</strong>, <strong>parcial_tarde</strong>, <strong>eja</strong></li>
            <li>Você pode informar múltiplos tipos separados por vírgula (ex: lanche_manha,almoco,lanche_tarde)</li>
            <li>O campo <strong>ativo</strong> deve ser 1 (ativo) ou 0 (inativo). Se não informado, será considerado 1</li>
            <li>Se já existir um registro para a escola, ele será atualizado</li>
          </ul>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-blue-600">{resultado.total || 0}</div>
                  <div className="text-sm text-gray-600">Total de Linhas</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-600">{resultado.sucesso?.length || 0}</div>
                  <div className="text-sm text-gray-600">Importadas</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-red-600">{resultado.erros?.length || 0}</div>
                  <div className="text-sm text-gray-600">Erros</div>
                </div>
              </div>
            </div>

            {/* Vínculos importados com sucesso */}
            {resultado.sucesso && resultado.sucesso.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  <FaCheckCircle className="text-green-600" />
                  <h4 className="font-semibold text-green-900">
                    Vínculos Importados ({resultado.sucesso.length})
                  </h4>
                </div>
                <div className="max-h-40 overflow-y-auto space-y-1">
                  {resultado.sucesso.map((item, index) => (
                    <div key={index} className="text-sm text-green-800 flex justify-between">
                      <span>Linha {item.linha}: {item.escola_nome} - {item.tipos_atendimento.join(', ')} ({item.acao})</span>
                    </div>
                  ))}
                </div>
              </div>
...
export default ImportTipoAtendimentoEscolaModal;
