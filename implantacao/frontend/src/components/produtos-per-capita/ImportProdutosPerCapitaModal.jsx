...
const ImportProdutosPerCapitaModal = ({ isOpen, onClose, onImportSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await ProdutosPerCapitaService.importar(formData);

      if (response.success) {
        setResultado(response.data);
        
        // Se teve sucesso parcial ou total, chamar callback
        if (response.data?.detalhes?.sucesso?.length > 0) {
          toast.success(`Importação concluída: ${response.data.detalhes.sucesso.length} produto(s) processado(s)`);
          setTimeout(() => {
            onImportSuccess();
          }, 2000);
        } else if (response.data?.detalhes?.erros?.length > 0) {
          toast.error(`Nenhum produto foi importado. ${response.data.detalhes.erros.length} erro(s) encontrado(s)`);
        }
      } else {
        setErro(response.message || response.error || 'Erro ao importar planilha');
        toast.error(response.message || response.error || 'Erro ao importar planilha');
      }
    } catch (error) {
      console.error('Erro ao importar:', error);
      const errorMessage = error.response?.data?.message || error.message || 'Erro ao processar importação';
  ...
  const handleDownloadModelo = async () => {
    try {
      const response = await ProdutosPerCapitaService.baixarModelo();
      if (response.success) {
        toast.success('Modelo baixado com sucesso!');
      } else {
        setErro(response.error || 'Erro ao baixar modelo de planilha');
        toast.error(response.error || 'Erro ao baixar modelo de planilha');
      }
    } catch (error) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Importar Produtos Per Capita"
      size="3xl"
    >
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-blue-600">{resultado.detalhes.total}</div>
                  <div className="text-sm text-gray-600">Total de Linhas</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-600">{resultado.detalhes.sucesso.length}</div>
                  <div className="text-sm text-gray-600">Processadas</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-red-600">{resultado.detalhes.erros.length}</div>
            ...
            {resultado.detalhes.sucesso.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  <FaCheckCircle className="text-green-600" />
                  <h4 className="font-semibold text-green-900">
                    Produtos Processados ({resultado.detalhes.sucesso.length})
                  </h4>
                </div>
                <div className="max-h-40 overflow-y-auto space-y-1">
                  {resultado.detalhes.sucesso.map((item, index) => (
                    <div key={index} className="text-sm text-green-800 flex justify-between">
                      <span>Linha {item.linha}: {item.produto} - <span className="text-green-600">{item.acao}</span></span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-semibold text-red-900">
                    Erros Encontrados ({resultado.detalhes.erros.length})
                  </h4>
                </div>
                <div className="max-h-40 overflow-y-auto space-y-2">
                  {resultado.detalhes.erros.map((item, index) => (
                    <div key={index} className="text-sm bg-white rounded p-2">
                      <div className="font-medium text-red-800">Linha {item.linha} - {item.produto}:</div>
...
export default ImportProdutosPerCapitaModal;
