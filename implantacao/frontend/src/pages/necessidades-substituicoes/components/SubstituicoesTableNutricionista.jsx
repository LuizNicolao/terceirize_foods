...
import { FaChevronDown, FaChevronUp, FaSave, FaCheckCircle } from 'react-icons/fa';
...
const SubstituicoesTableNutricionista = ({
  ...
  // Verificar se ajustes já foram ativados
  useEffect(() => {
  ...
  // Pré-selecionar produtos quando carregados
  useEffect(() => {
  ...
  const handleLiberarParaCoordenacao = async () => {
    ...
    try {
      const response = await onLiberarParaCoordenacao(necessidades);
      if (response && response.success) {
        toast.success('Necessidades liberadas para coordenação com sucesso!');
        setAjustesAtivados(true);
      }
    } catch (error) {
      toast.error(`Erro ao liberar para coordenação: ${error.message}`);
    }
  };
  ...
  if (necessidades.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
        <p className="text-gray-600">Nenhuma necessidade encontrada para ajuste nutricionista.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Botão Liberar para Coordenação */}
      {ajustesAtivados && canEdit && (
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-green-600">
                {necessidades.length} produto(s) com substituições configuradas
              </p>
            </div>
            <Button
              variant="success"
              onClick={handleLiberarParaCoordenacao}
                    ...
                    <tr className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 whitespace-nowrap">
                        <SearchableSelect
                          ...
                          disabled={!ajustesAtivados || loadingGenericos[necessidade.codigo_origem] || !canEdit}
                          className="text-xs"
                                    ...
                                    <tr key={`${escola.escola_id}-${idx}`}>
                                      ...
                                      <td className="px-4 py-2 whitespace-nowrap">
                                        <SearchableSelect
                                          ...
                                          placeholder="Selecione..."
                                          disabled={!ajustesAtivados || !canEdit}
                                          filterBy={(option, searchTerm) => {
                                            return option.label.toLowerCase().includes(searchTerm.toLowerCase());
                                          }}
                                        />
