...
const SubstituicoesTableCoordenacao = ({
  ...
  const handleAprovar = async (necessidade) => {
    try {
      const response = await onAprovar(necessidade);
      if (response && response.success) {
  ...
  const handleRejeitar = async (necessidade) => {
    try {
      const response = await onRejeitar(necessidade);
      if (response && response.success) {
  ...
  if (necessidades.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
        <p className="text-gray-600">Nenhuma necessidade encontrada para ajuste de coordenação.</p>
      </div>
    );
  }
                  ...
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 whitespace-nowrap">
                      <SearchableSelect
                        ...
                        disabled={loadingGenericos[necessidade.codigo_origem] || !canEdit}
                        className="text-xs"
                        ...
                        {canApprove && (
                          <>
                            <Button
                              size="xs"
                              variant="primary"
                              onClick={() => handleAprovar(necessidade)}
                              className="flex items-center gap-1"
                            >
                            ...
                            <Button
                              size="xs"
                              variant="danger"
                              onClick={() => handleRejeitar(necessidade)}
                              className="flex items-center gap-1"
                            >
                          ...
                          <table className="min-w-full divide-y divide-gray-200">
                            <thead>
                              <tr className="bg-gray-100">
                                ...
                                <th style={{ width: '200px' }} className="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase">Ações</th>
                              </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
