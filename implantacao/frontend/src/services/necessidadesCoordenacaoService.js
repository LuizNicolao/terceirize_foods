...
const necessidadesCoordenacaoService = {
  ...
  async salvarAjustesCoordenacao(itens) {
    try {
      const response = await api.put('/necessidades/coordenacao/ajustes', { itens });
      return response.data;
    } catch (error) {
  ...
  async liberarParaLogistica(necessidade_ids) {
    try {
  ...
  async buscarProdutosParaModal(filtros = {}) {
    try {
      ...
      const response = await api.get(`/necessidades/coordenacao/produtos?${params.toString()}`);
      return response.data;
    } catch (error) {
