    ...
    try {
      ...
      const query = `
        SELECT DISTINCT
          ...
          u.atualizado_em 
        FROM usuarios u
        -- INNER JOIN usuarios_filiais uf ON u.id = uf.usuario_id -- TODO: Implementar quando a tabela filiais for criada
        WHERE u.tipo_de_acesso = ? 
          AND u.status = 'ativo'
          -- AND uf.filial_id = ? -- TODO: Implementar quando a tabela filiais for criada
        ORDER BY u.nome ASC
      `;
      
      const usuarios = await executeQuery(query, [tipo]); // Removido filialId temporariamente
