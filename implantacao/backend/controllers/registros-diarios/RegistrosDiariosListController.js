...
class RegistrosDiariosListController {
  ...
  static async listar(req, res) {
    try {
      ...
      // Subquery para pegar a data mais recente de cada escola
      const query = `
        SELECT 
          rd.escola_id,
        ...
        INNER JOIN (
          ...
          ${whereClause}
          GROUP BY escola_id
        ) latest ON rd.escola_id = latest.escola_id AND rd.data = latest.max_data
  ...
  /**
   * Listar histórico de registros de uma escola
   */
  static async listarHistorico(req, res) {
    try {
      ...
      // Buscar todos os registros da escola, agrupados por data
      const query = `
        SELECT 
          rd.escola_id,
          MAX(rd.escola_nome) as escola_nome,
