...
const ModalValidacaoExclusao = ({ 
  ...
  const carregarDiasComRegistros = async () => {
    try {
      setLoading(true);
      const response = await quantidadesServidasService.listarHistorico(unidadeId);
  ...
  const handleConfirmarExclusao = async () => {
    ...
    try {
      ...
      const promises = Array.from(diasSelecionados).map(data => 
        quantidadesServidasService.excluir(unidadeId, data)
      );
      ...
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="font-semibold text-blue-900">{unidadeNome}</h3>
