            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {produtos.map((produto) => (
                <tr key={produto.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {produto.unidade_medida_nome || produto.unidade_medida || '-'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {produto.grupo_nome || '-'}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end">
                      <ActionButtons
                        canView={canView}
                        canEdit={false} // Apenas consulta
                        canDelete={false} // Apenas consulta
                        onView={() => onView && onView(produto)}
                        onEdit={() => {}}
                        onDelete={() => {}}
                        item={produto.id}
                        size="xs"
          ...
          <div key={produto.id} className="bg-white rounded-lg shadow-sm p-4 border">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                ...
                <p className="font-medium">{produto.unidade_medida_nome || produto.unidade_medida || '-'}</p>
              </div>
              <div>
                <span className="text-gray-500">Grupo:</span>
                <p className="font-medium">{produto.grupo_nome || '-'}</p>
              </div>
              <div>
                <span className="text-gray-500">Subgrupo:</span>
                <p className="font-medium">{produto.subgrupo_nome || '-'}</p>
              </div>
              <div>
                <span className="text-gray-500">Classe:</span>
                <p className="font-medium">{produto.classe_nome || '-'}</p>
              </div>
              <div>
