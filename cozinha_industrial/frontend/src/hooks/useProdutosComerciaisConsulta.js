...
/**
 * Hook para consultar produtos comerciais do sistema Foods
 * Modo apenas leitura - sem operações CRUD
 */
export const useProdutosComerciaisConsulta = () => {
  // Estados principais
  ...
  /**
   * Calcular estatísticas dos produtos comerciais
   */
  ...
  const carregarProdutos = useCallback(async () => {
    ...
    try {
      ...
      // Buscar todos os produtos comerciais com paginação múltipla
      let allProdutos = [];
      let currentPage = 1;
      ...
      while (hasMorePages && currentPage <= maxPages) {
        const result = await FoodsApiService.getProdutosComerciais({
          page: currentPage,
  ...
  const atualizarFiltros = useCallback((novosFiltros) => {
    ...
    if (novosFiltros.search !== undefined || filters.search !== '') {
      ...
      dadosFiltrados = dadosFiltrados.filter(produto => 
        produto.nome_comercial?.toLowerCase().includes(searchTerm) ||
