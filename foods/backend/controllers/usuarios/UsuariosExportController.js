...
class UsuariosExportController {
  static async exportarXLSX(req, res) {
    try {
      ...
      const { search, status, tipo, limit = 1000 } = req.query;
      let where = 'WHERE 1=1';
      const params = [];
      ...
      if (tipo && tipo !== 'todos') { where += ' AND tipo = ?'; params.push(tipo); }

      const usuarios = await executeQuery(`SELECT id, nome, email, tipo, status FROM usuarios ${where} ORDER BY nome ASC LIMIT ${parseInt(limit)}`, params);
      usuarios.forEach(u => ws.addRow({ id: u.id, nome: u.nome, email: u.email, tipo: u.tipo, status: u.status === 1 ? 'Ativo' : 'Inativo' }));
