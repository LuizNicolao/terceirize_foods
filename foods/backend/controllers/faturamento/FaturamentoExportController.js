...
class FaturamentoExportController {
  ...
  /**
   * Imprimir faturamento específico em PDF
   */
  static async imprimirPDF(req, res) {
    try {
      const { id } = req.params;
      ...
      // Calcular total
      const totalRefeicoes = detalhes.reduce((total, dia) => {
        return total + (parseInt(dia.desjejum) || 0) + 
               (parseInt(dia.lanche_matutino) || 0) + 
               (parseInt(dia.almoco) || 0) + 
               (parseInt(dia.lanche_vespertino) || 0) + 
               (parseInt(dia.noturno) || 0);
      }, 0);
      ...
      // Configurar resposta
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=faturamento_${id}_${new Date().toISOString().split('T')[0]}.pdf`);

      doc.pipe(res);

      // Cabeçalho
      doc.fontSize(20).text('Faturamento de Refeições', { align: 'center' });
      doc.moveDown();
      doc.fontSize(12).text(`Gerado em: ${new Date().toLocaleDateString('pt-BR')}`, { align: 'center' });
      doc.moveDown(2);
      ...
      doc.text(`Código: ${faturamento.codigo_teknisa}`);
      doc.text(`Período: ${faturamento.mes}/${faturamento.ano}`);
      doc.text(`Total de Refeições: ${totalRefeicoes.toLocaleString('pt-BR')}`);
      doc.text(`Cidade: ${faturamento.cidade} - ${faturamento.estado}`);
      ...
      // Tabela de detalhes
      doc.fontSize(14).text('Detalhamento por Dia', { underline: true });
      doc.moveDown();

      // Cabeçalho da tabela
      const tableTop = doc.y;
      const itemHeight = 20;
      const colWidths = [50, 60, 60, 60, 60, 60];

      doc.fontSize(10);
      doc.text('Dia', 50, tableTop);
      doc.text('Desjejum', 100, tableTop);
      doc.text('Lanche Mat.', 160, tableTop);
      doc.text('Almoço', 220, tableTop);
      doc.text('Lanche Ves.', 280, tableTop);
      doc.text('Noturno', 340, tableTop);

      // Linha separadora
      doc.moveTo(50, tableTop + 15).lineTo(400, tableTop + 15).stroke();

      // Dados da tabela
      detalhes.forEach((dia, index) => {
        const y = tableTop + 20 + (index * itemHeight);
        
        doc.text(dia.dia.toString(), 50, y);
        doc.text((dia.desjejum || 0).toString(), 100, y);
        doc.text((dia.lanche_matutino || 0).toString(), 160, y);
        doc.text((dia.almoco || 0).toString(), 220, y);
        doc.text((dia.lanche_vespertino || 0).toString(), 280, y);
        doc.text((dia.noturno || 0).toString(), 340, y);
      });

      doc.end();

    } catch (error) {
      console.error('Erro ao imprimir faturamento:', error);
      res.status(500).json({
        success: false,
        message: 'Erro interno do servidor',
        error: process.env.NODE_ENV === 'development' ? error.message : undefined
