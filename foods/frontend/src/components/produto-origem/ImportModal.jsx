...
const ImportModal = ({ isOpen, onClose, onImportSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await ProdutoOrigemService.importarExcel(formData);
  ...
  const handleDownloadModelo = async () => {
    try {
      await ProdutoOrigemService.baixarModelo();
    } catch (error) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Importar Produtos Origem"
      size="3xl"
    >
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
            ...
            <li>O <strong>código será gerado automaticamente</strong> pelo sistema</li>
            <li>Os campos <strong>nome</strong> e <strong>unidade_medida</strong> são obrigatórios</li>
            <li>Certifique-se de que grupos, subgrupos e classes existem no sistema</li>
          </ul>
        </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-semibold text-green-900">
                    Produtos Importados ({resultado.sucesso.length})
                  </h4>
                </div>
                <div className="max-h-40 overflow-y-auto space-y-1">
                  {resultado.sucesso.map((item) => (
                    <div key={item.id} className="text-sm text-green-800 flex justify-between">
                      <span>Linha {item.linha}: {item.nome}</span>
                      <span className="font-mono text-green-600">{item.codigo}</span>
                    </div>
                  ))}
                </div>
              </div>
