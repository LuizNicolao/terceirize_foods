  ...
  useEffect(() => {
    const carregarProximoCodigo = async () => {
      if (produto && isOpen) {
        ...
        try {
          const response = await produtosService.obterProximoCodigo();
          if (response.success) {
            setValue('codigo_produto', response.data.proximoCodigo);
          } else {
            setValue('codigo_produto', 'Erro ao gerar código');
          }
        } catch (error) {
