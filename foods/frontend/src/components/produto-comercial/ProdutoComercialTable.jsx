...
const ProdutoComercialTable = ({
  ...
  if (!produtosComerciais || produtosComerciais.length === 0) {
    return (
      <EmptyState
        title="Nenhum produto comercial encontrado"
        description="Não há produtos comerciais cadastrados ou os filtros aplicados não retornaram resultados"
        icon="produto-comercial"
      />
    );
  }
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {produtosComerciais.map((produto) => (
                <tr key={produto.id} className="hover:bg-gray-50">
                  <td className="px-6 py-2 whitespace-nowrap text-sm font-medium text-gray-900">
                    {produto.codigo ? produto.codigo.replace('COM-', '') : '-'}
                  </td>
                  <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-900">
                    <div className="font-medium">{produto.nome_comercial}</div>
                  </td>
                  <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-900">
          ...
          <div key={produto.id} className="bg-white rounded-lg shadow-sm p-4 border">
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1">
                <h3 className="font-semibold text-gray-900 text-sm">{produto.nome_comercial}</h3>
                <p className="text-gray-600 text-xs">
                  Código: {produto.codigo ? produto.codigo.replace('COM-', '') : '-'}
                </p>
              </div>
              <ActionButtons
                canView={canView}
...
export default ProdutoComercialTable;
