...
const Produtos = () => {
  ...
  const getGrupoName = (grupoId) => {
    const grupo = Array.isArray(grupos) ? grupos.find(g => g.id === grupoId) : null;
    ...
    <div className="p-6">
      ...
      <CadastroFilterBar
        ...
        filters={[
          ...
          { key: 'grupo_id', label: 'Grupo', type: 'select', options: Array.isArray(grupos) ? grupos.map(g => ({ value: g.id, label: g.nome_grupo })) : [] },
          { key: 'unidade_id', label: 'Unidade', type: 'select', options: Array.isArray(unidades) ? unidades.map(u => ({ value: u.id, label: u.nome_unidade })) : [] }
        ]}
      />
