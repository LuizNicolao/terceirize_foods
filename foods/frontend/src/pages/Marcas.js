...
const Marcas = () => {
  ...
  const loadMarcas = async () => {
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Erro ao carregar marcas');
        setMarcas([]);
      }
    } catch (error) {
  ...
  const loadEstatisticas = async () => {
    try {
      const result = await MarcasService.listar({ limit: 1000 });
      if (result.success) {
        const data = Array.isArray(result.data) ? result.data : [];
    ...
    try {
      if (editingMarca) {
        ...
        if (result.success) {
          ...
          toast.error(result.error || 'Erro ao atualizar marca');
        }
      } else {
        const result = await MarcasService.criar(data);
        if (result.success) {
          ...
          toast.error(result.error || 'Erro ao criar marca');
        }
      }
    } catch (error) {
    ...
    if (window.confirm('Tem certeza que deseja excluir esta marca?')) {
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.error || 'Erro ao excluir marca');
        }
      } catch (error) {
  ...
  const handleExportXLSX = async () => {
    try {
      ...
      const result = await MarcasService.exportarXLSX(params);
      if (result.success) {
        const blob = result.data;
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `marcas_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const result = await MarcasService.exportarPDF(params);
      if (result.success) {
        const blob = result.data;
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `marcas_${new Date().toISOString().split('T')[0]}.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
