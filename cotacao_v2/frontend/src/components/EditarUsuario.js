...
const EditarUsuario = () => {
  ...
  const fetchUsuario = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/users/${id}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (response.ok) {
          alert(id === 'new' ? 'Usuário criado com sucesso!' : 'Usuário atualizado com sucesso!');
          navigate('/usuarios');
        } else {
          const errorData = await response.json();
