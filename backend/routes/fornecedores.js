...
router.post('/import', [
  ...
  try {
    ...
    if (!Array.isArray(fornecedores) || fornecedores.length === 0) {
      return res.status(400).json({ error: 'Dados de fornecedores inválidos' });
    }
    ...
    // Campos válidos da tabela fornecedores
    const camposValidos = [
      'cnpj', 'razao_social', 'nome_fantasia', 'logradouro', 'numero', 
      'cep', 'bairro', 'municipio', 'uf', 'email', 'telefone', 'status'
    ];
    ...
    for (let i = 0; i < fornecedores.length; i++) {
      ...
      try {
        ...
        camposValidos.forEach(campo => {
          if (fornecedor[campo] !== undefined && fornecedor[campo] !== null && fornecedor[campo] !== '') {
            dadosFiltrados[campo] = fornecedor[campo];
          }
        });

        // Validações obrigatórias
        if (!dadosFiltrados.cnpj) {
          throw new Error('CNPJ é obrigatório');
        }

        if (!dadosFiltrados.razao_social) {
          throw new Error('Razão social é obrigatória');
        }

        // Validar CNPJ
        const cnpjLimpo = dadosFiltrados.cnpj.toString().replace(/\D/g, '');
        if (cnpjLimpo.length !== 14) {
          throw new Error('CNPJ deve ter 14 dígitos');
        }
        ...
        if (dadosFiltrados.email) {
          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
          if (!emailRegex.test(dadosFiltrados.email)) {
            throw new Error('Email inválido');
          }
        }

        // Validar UF se fornecida
        if (dadosFiltrados.uf && dadosFiltrados.uf.length !== 2) {
          throw new Error('UF deve ter 2 caracteres');
        }

        // Definir status padrão se não fornecido
        if (!dadosFiltrados.status) {
          dadosFiltrados.status = 1; // Ativo por padrão
        }
        ...
        // Inserir fornecedor
        await executeQuery(
          `INSERT INTO fornecedores (${campos.join(', ')}) VALUES (${placeholders})`,
          valores
        );
