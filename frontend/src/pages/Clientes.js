...
const Clientes = () => {
  ...
  const loadAuditLogs = async () => {
    try {
      ...
      // Adicionar filtro específico para clientes
        ...
        try {
          ...
          for (let i = 0; i < cnpjs.length; i++) {
            ...
            try {
              ...
              const response = await api.get(`/clientes/buscar-cnpj/${cnpj.limpo}`);
              ...
              if (response.data.success && response.data.data) {
                ...
                // Cadastrar cliente
                const createResponse = await api.post('/clientes', dadosCliente);
          ...
          setImportResults({
            resultados,
            mensagem: `Importação concluída! ${resultados.sucessos} clientes importados com sucesso e ${resultados.erros} erros.`,
            estatisticas: {
  ...
  const onSubmit = async (data) => {
    try {
      if (editingCliente) {
        ...
        if (data.razao_social !== editingCliente.razao_social) {
          updateData.razao_social = data.razao_social;
        }
        
        if (data.nome_fantasia !== editingCliente.nome_fantasia) {
          updateData.nome_fantasia = data.nome_fantasia;
        }
        
        if (data.cnpj !== editingCliente.cnpj) {
          updateData.cnpj = data.cnpj;
        }
        
        if (data.email !== editingCliente.email) {
          updateData.email = data.email;
        }
        
        if (data.telefone !== editingCliente.telefone) {
          updateData.telefone = data.telefone;
        }
        
        if (data.uf !== editingCliente.uf) {
          updateData.uf = data.uf;
        }
        
        if (data.municipio !== editingCliente.municipio) {
        ...
        await api.put(`/clientes/${editingCliente.id}`, updateData);
        ...
        await api.post('/clientes', createData);
    ...
    return sortClientes(
      clientes.filter(cliente => {
        ...
        if (searchTerm) {
          switch (searchField) {
            case 'id':
              matchesSearch = cliente.id?.toString().includes(searchTerm);
              break;
            case 'razao_social':
              matchesSearch = cliente.razao_social?.toLowerCase().includes(searchTerm.toLowerCase());
              break;
            case 'nome_fantasia':
              matchesSearch = cliente.nome_fantasia?.toLowerCase().includes(searchTerm.toLowerCase());
              break;
            case 'cnpj':
              matchesSearch = cliente.cnpj?.includes(searchTerm);
              break;
            case 'email':
              matchesSearch = cliente.email?.toLowerCase().includes(searchTerm.toLowerCase());
              break;
            case 'telefone':
              matchesSearch = cliente.telefone?.includes(searchTerm);
              break;
            case 'municipio':
              matchesSearch = cliente.municipio?.toLowerCase().includes(searchTerm.toLowerCase());
              break;
            case 'uf':
              matchesSearch = cliente.uf?.toLowerCase().includes(searchTerm.toLowerCase());
              break;
            case 'todos':
            default:
              matchesSearch = cliente.razao_social?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         cliente.cnpj?.includes(searchTerm) ||
                             ...
                             cliente.uf?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                             cliente.id?.toString().includes(searchTerm);
              break;
          }
        }
        
    const matchesStatus = statusFilter === 'todos' || cliente.status === parseInt(statusFilter);
    return matchesSearch && matchesStatus;
      })
    );
  }, [clientes, searchTerm, searchField, statusFilter, sortField, sortDirection]);
  ...
  const buscarDadosCNPJ = async (cnpj) => {
    try {
      ...
      const response = await api.get(`/clientes/buscar-cnpj/${cnpjLimpo}`);
  ...
  const handleCNPJChange = (e) => {
    ...
    if (cnpjLimpo.length === 14 && !editingCliente) {
            ...
            {filteredClientes.length === 0 ? (
              ...
              currentItems.map((cliente) => (
                <tr key={cliente.id}>
                  ...
                  <Td>
                    <StatusBadge status={cliente.status === 1 ? 'ativo' : 'inativo'}>
      ...
      {showModal && (
        <Modal onClick={handleCloseModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <ModalHeader>
              <ModalTitle>
                {isViewMode ? 'Visualizar cliente' : editingCliente ? 'Editar cliente' : 'Adicionar cliente'}
              </ModalTitle>
      ...
      {showAuditModal && (
        <Modal onClick={handleCloseAuditModal}>
          <ModalContent onClick={(e) => e.stopPropagation()} style={{ maxWidth: '95vw', maxHeight: '90vh', width: '1200px' }}>
            <ModalHeader>
              <ModalTitle>Relatório de Auditoria - Clientes</ModalTitle>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <button
            ...
            {!importResults && (
              <div style={{ 
                ...
                <ul style={{ 
                  ...
                  <li>Clientes são cadastrados com status "Ativo"</li>
                ...
                {importResults.resultados.detalhes.sucessos.length > 0 && (
                  <div style={{ marginBottom: '20px' }}>
                    <h4 style={{ 
                      ...
                      Clientes Importados com Sucesso ({importResults.resultados.detalhes.sucessos.length})
                    </h4>
                    <div style={{ 
                      maxHeight: '200px', 
                      overflowY: 'auto',
